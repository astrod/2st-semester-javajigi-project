...
public class LadderGameMaster {
	...
	private void goPath(LadderCol startObject, int startIndex, LadderMap map) {
		...
		if(startObject.ladderArray[startIndex] == -1) goPath(startObject.preLadder, startIndex+1, map);		
		
		if(startObject.ladderArray[startIndex] == -2) goPath(startObject.postLadder, startIndex+1, map);
